...
__global__ void MonteCarloKernel(
    float* S, float K, float T, float r, float v, float* z, float *C)
{
...
int main()
{
   ...
   //   1. allocate and initialize CPU memory
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        curandGenerateNormal(gen, d_z, N, 0.0f, 1.0f); 
        ...
        MonteCarloKernel<<<gridSize, blockSize>>>(
                    d_S, K, T_values[i], r_values[i], v, d_z, d_C);
