...
// Function to calculate option prices and Greeks parallel
void calculate_option_thread(float* K, float* T, float* S0, float* sigma, float* r, 
                      float* C_price, float* P_price, 
                      float* C_delta, float* P_delta,
                      float* C_gamma, float* P_gamma, 
                      float* C_vega, float* P_vega,
                      ...
                      int NumOptions)
{
    ...
    auto worker = [K, T, S0, sigma, r, &C_price, &P_price, 
                   &C_delta, &P_delta, &C_gamma, &P_gamma, 
                   &C_vega, &P_vega, &C_rho, &P_rho, 
                   &C_theta, &P_theta, NumOptions](int start, int end) {
        for (int i = start; i < end; ++i) {
            if (i >= NumOptions) break; // Ensure the index is within bounds
    ...
    int options_per_thread = (NumOptions + num_threads - 1) / num_threads; // Ensure even distribution of work

    for (int i = 0; i < num_threads; ++i) {
        int start = i * options_per_thread;
        int end = start + options_per_thread; // Ensure end is within bounds
...
// Function to calculate option prices and Greeks parallel
void calculate_option_omp(float* K, float* T, float* S0, float* sigma, float* r, 
                      float* C_price, float* P_price, 
                      float* C_delta, float* P_delta,
                      float* C_gamma, float* P_gamma, 
                      float* C_vega, float* P_vega,
...
int main() 
{
    ...
    calculate_option_thread(K, T, S0, sigma, r, 
                            C_price, P_price, 
                            C_delta, P_delta, 
                            C_gamma, P_gamma, 
                            C_vega, P_vega, 
    ...
    // calculate_option_omp(K, T, S0, sigma, r, 
    //                      C_price, P_price, 
    //                      C_delta, P_delta, 
    //                      C_gamma, P_gamma, 
    //                      C_vega, P_vega, 
