...
def initialize_option_values(np.ndarray[np.float64_t, ndim=2] asset_prices, 
    ...
    for j in prange(N + 1, nogil=True, schedule="static"):
        if option_type == 'call':
...
def backtrack_option_values(np.ndarray[np.float64_t, ndim=2] option_values, 
    ...
    """
    cdef unsigned int i, j
    cdef float exp_factor = exp(-r * dt)
...
#    return option_values[0, 0]
    ...
    for i in range(N - 1, -1, -1):
        option_values[:i+1, i] = exp_factor * (p * option_values[:i+1, i + 1]
                                               + (1 - p) * option_values[1:i+2, i + 1])
