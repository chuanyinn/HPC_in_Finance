...
void MonteCarloKernel(
    const vector<float>& S, float K, float T, float r, float v, vector<float>& C)
{
    ...
    for (int idx = 0; idx < N; ++idx) {
        ...
        unsigned seed = rd() + idx;
        mt19937 gen(seed);
        float gauss_bm = distribution(gen);
...
int main()
{
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        MonteCarloKernel(h_S, K, T_values[i], r_values[i], v, h_C);
